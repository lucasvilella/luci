...
stdin, stdout, stderr = ssh.exec_command('ls -la /root/luci-server/server.log ~/cf_live.log')
print("LS:\n", stdout.read().decode('utf-8', errors='ignore').encode('ascii', 'ignore').decode('ascii'))
print("STDERR:\n", stderr.read().decode('utf-8', errors='ignore').encode('ascii', 'ignore').decode('ascii'))
