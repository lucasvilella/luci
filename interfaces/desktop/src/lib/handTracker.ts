...
export interface HandTrackerCallbacks {
  ...
  onFaceTrack?(x: number, y: number): void;
}
...
export class HandTracker {
  ...
  private faceRecognizer = new FaceRecognizerManager();
  ...
  private lastStatus: TrackerStatus = { hands: 0, faceDetected: false, mode: "idle" };
  ...
  async start(): Promise<void> {
    ...
    try {
      this.landmarker = await HandLandmarker.createFromOptions(fileset, handOptions);
    } catch {
      this.landmarker = await HandLandmarker.createFromOptions(fileset, {
        ...handOptions,
        baseOptions: { ...handOptions.baseOptions, delegate: "CPU" as const },
      });
    }

    try {
      this.faceLandmarker = await FaceLandmarker.createFromOptions(fileset, faceOptions);
    } catch (e) {
      console.warn("FaceLandmarker fallback to CPU:", e);
      try {
        this.faceLandmarker = await FaceLandmarker.createFromOptions(fileset, {
          ...faceOptions,
          baseOptions: { ...faceOptions.baseOptions, delegate: "CPU" as const },
        });
      } catch (err) {
  ...
  stop(): void {
    ...
    this.emitStatus({ hands: 0, faceDetected: false, mode: "idle" });
  }
  ...
  private loop = () => {
    ...
    if (this.landmarker) {
      ...
      handLabels = result.handedness.map((h) => h[0]?.categoryName ?? "?");
    }

    let recognizedName = "Lucas";
    ...
    if (this.faceLandmarker) {
      try {
        ...
        if (faceResult.faceLandmarks && faceResult.faceLandmarks.length > 0) {
          ...
          if (nose && this.callbacks.onFaceTrack) {
            const dx = (0.5 - nose.x) * 0.4;
            const dy = (nose.y - 0.5) * 0.4;
            this.callbacks.onFaceTrack(dx, dy);
          }

          const liveVector = FaceRecognizerManager.extractVector(landmarks);
          if (liveVector) {
            const matchResult = this.faceRecognizer.match(liveVector);
            if (matchResult.matched) {
              recognizedName = matchResult.name;
            }
          }
        }
      } catch {
    ...
    this.processHands(handLandmarks, handLabels, faceDetected, recognizedName);
  ...
  private processHands(
    ...
    this.emitStatus({
      ...
      faceDetected,
      faceName: faceDetected ? recognizedName : undefined,
      mode,
    });
  }
  ...
  private emitStatus(status: TrackerStatus): void {
    if (
      ...
      status.faceDetected !== this.lastStatus.faceDetected ||
